...
router.get('/wallet', GetValidators.isLoggedAndConfigured, (req: IExtendedRequest, res: express.Response) => {
    ...
    AdminSteemConnect.me(function (err: Error, user: any) {
        if (!err && user) {
...
router.post('/claim', GetValidators.isLoggedAndConfigured, (req: IExtendedRequest, res: express.Response) => {
    ...
    AdminSteemConnect.me(function (err: Error, user: any) {
        if (err) {
            ...
            AdminSteemConnect.claimRewardBalance(user.name, user.account.reward_steem_balance, user.account.reward_sbd_balance, user.account.reward_vesting_balance, function (err: Error) {
                if (err) {
                    res.json({ error: "Error while claiming rewards" })
                } else {
