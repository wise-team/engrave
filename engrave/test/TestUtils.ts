...
export class TestUtils {
    ...
    static async login(access_token: string) {
        try {
            let result: any = await request(server).get("/authorize?access_token=" + access_token);
    ...
    private static mockSteemconnect() {
        nock('https://steemconnect.com/api')
            .matchHeader('authorization', 'configured')
            .post('/me')
            .reply(200, this.configuredUser);

        nock('https://steemconnect.com/api')
            .matchHeader('authorization', 'not_configured')
            .post('/me')
            .reply(200, this.notConfiguredUser);
    }
